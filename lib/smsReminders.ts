...
export async function recordPaidOrderAudienceBySession(sessionId:string){
  ...
  const result=await db().query(
    "SELECT customer,created_at FROM orders WHERE stripe_session_id=$1 LIMIT 1",
    [sessionId]
  );
...
export async function setAudienceSubscriptionByEmail(email:string,active:boolean){
  ...
  if(!email) return;
  await db().query(
    "UPDATE sms_audience SET active_subscription=$2,updated_at=now() WHERE lower(email)=lower($1)",
    [email,active]
  );
}
...
export async function reminderCandidates(at=DateTime.now().setZone(ZONE)){
  ...
  const lookbackDays=Math.max(1,Number.parseInt(process.env.SMS_REMINDER_LOOKBACK_DAYS||"90",10)||90);
  const lookbackStart=at.minus({days:lookbackDays});
  const result=await db().query(
...
export function clickSendConfigured(){
  return Boolean(process.env.CLICKSEND_USERNAME&&process.env.CLICKSEND_API_KEY);
}

function renderMessage(row:{first_name?:string;last_name?:string}){
  const template=process.env.SMS_REMINDER_MESSAGE||
    "Hi {first_name}, this week's Simple Kitchen menu is live. Order at {shop_url}";
  return template
...
async function sendClickSend(to:string,body:string){
  if(!clickSendConfigured()) throw new Error("ClickSend is not configured.");
  const auth=Buffer.from(process.env.CLICKSEND_USERNAME+":"+process.env.CLICKSEND_API_KEY).toString("base64");
  const message:{body:string;to:string;source:string;from?:string}={
    body,
    to,
...
export async function runSmsReminders(options:{dryRun?:boolean}={}){
  const candidates=await reminderCandidates();
  if(options.dryRun){
    return {
      dryRun:true,
      weekStart:candidates.weekStart,
  ...
  for(const row of candidates.rows){
    try{
      await sendClickSend(row.phone_normalized,renderMessage(row));
      await db().query(
